...
static int cdp_ctr(struct dm_target *ti, unsigned int argc, char **argv)
{
	...
	DMINFO("attached %s +%llu (%llu sectors) as /dev/cdp%d, ring %u MiB, chunk %u KiB",
	       t->dev->name, (unsigned long long)t->start,
	       (unsigned long long)ti->len, t->minor, ring_mb, chunk_kb);
	return 0;
